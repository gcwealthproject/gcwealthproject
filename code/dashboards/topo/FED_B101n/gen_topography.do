...
local general_source = "FED_B101n" 
qui local sector_list S15 
qui local country_list US 

global grid "${topo_dir_raw}/FED_B101n/intermediate"
global bycountry_data "${topo_dir_raw}/FED_B101n/intermediate/populated_grid"
global cmappings "${topo_dir_raw}/FED_B101n/auxiliary files"
global intermediate "${topo_dir_raw}/FED_B101n/intermediate to erase"
...
keep if varcode == "p-np-agn-nnhass-ga" | varcode == "p-np-agn-nfahou-ga" | varcode == "p-np-agn-fliabi-lb"

save "${intermediate}/topo/country", replace

* Financial asset side
use "${intermediate}/topo/country", clear
keep if varcode == "p-np-agn-nnhass-ga"
rename value nnhass
save "${intermediate}/topo/nnhass", replace

* Housing asset side
use "${intermediate}/topo/country", clear
keep if varcode == "p-np-agn-nfahou-ga"
rename value nfahou
save "${intermediate}/topo/nfahou", replace

* Liabilities side
use "${intermediate}/topo/country", clear
keep if varcode == "p-np-agn-fliabi-lb"
rename value fliabi
...
rename netwea value
replace varcode = "p-np-agn-netwea-na"
...
global output "${topo_dir_raw}/FED_B101n/final table"
save "${output}/FED_B101n_warehouse.dta", replace
export delimited using "${output}/FED_B101n_warehouse.csv", replace
