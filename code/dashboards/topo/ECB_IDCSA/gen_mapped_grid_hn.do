...
// albania australia brazil canada chile colombia iceland israel  japan korea mexico newzealand northmacedonia norway russia switzerland turkey gb usa

	foreach c in albania australia brazil canada chile colombia iceland israel india ///
		...
		if "`c'" == "albania" {
			...
			qui replace na_code = subinstr(na_code, "US.", "", .) // Delete area quick fix ML24
		}		
		else if "`c'" == "india" {
			qui replace na_code = subinstr(na_code, "US.", "", .) // Delete area quick fix ML24
		}
		...
		export excel "${destination}/grid_hn", sheet("`c'_`s'", modify) firstrow(variables) 
