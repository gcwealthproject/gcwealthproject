...
local general_source = "BoI_NA" // The source does not change across the do-file
qui local sector_list S1M 
qui local country_list italy 

global grid "${topo_dir_raw}/BoI_NA/intermediate"
global bycountry_data "${topo_dir_raw}/BoI_NA/intermediate/populated_grid"
global cmappings "${topo_dir_raw}/BoI_NA/auxiliary files"
global intermediate "${topo_dir_raw}/BoI_NA/intermediate to erase"
...
keep if varcode == "p-hn-agn-nnhass-ga" | varcode == "p-hn-agn-nfahou-ga" | varcode == "p-hn-agn-fliabi-lb"

save "${intermediate}/topo/country", replace

* Financial asset side
use "${intermediate}/topo/country", clear
keep if varcode == "p-hn-agn-nnhass-ga"
rename value nnhass
save "${intermediate}/topo/nnhass", replace

* Housing asset side
use "${intermediate}/topo/country", clear
keep if varcode == "p-hn-agn-nfahou-ga"
rename value nfahou
save "${intermediate}/topo/nfahou", replace

* Liabilities side
use "${intermediate}/topo/country", clear
keep if varcode == "p-hn-agn-fliabi-lb"
rename value fliabi
...
rename netwea value
replace varcode = "p-hn-agn-netwea-na"
...
global output "${topo_dir_raw}/BoI_NA/final table"
save "${output}/BoI_NA_warehouse.dta", replace
export delimited using "${output}/BoI_NA_warehouse.csv", replace
